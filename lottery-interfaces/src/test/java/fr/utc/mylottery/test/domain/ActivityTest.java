...
public class ActivityTest {
    ...
    public void test_alterState() {
        ...
        logger.info("审核拒绝，测试：{}", JSON.toJSONString(stateHandler.checkRefuse(100001L, Constants.ActivityState.ARRAIGNMENT)));
        logger.info("审核拒绝后编辑，测试：{}", JSON.toJSONString(stateHandler.edit(100001L, Constants.ActivityState.REFUSE)));
        logger.info("二次提审，测试：{}", JSON.toJSONString(stateHandler.arraignment(100001L, Constants.ActivityState.EDIT)));
        logger.info("审核通过，测试：{}",JSON.toJSONString(stateHandler.checkPass(100001L,Constants.ActivityState.ARRAIGNMENT)));
        logger.info("活动进行，测试：{}",JSON.toJSONString(stateHandler.doing(100001L,Constants.ActivityState.PASS)));
